...
track_options = HomotopyContinuation.TrackerOptions(; parameters = :conservative, automatic_differentiation = 3, max_steps = 20_000, min_step_size = 1e-48)
...
track_options = HomotopyContinuation.TrackerOptions(; parameters = :fast, extended_precision = false)
end_options = HomotopyContinuation.EndgameOptions(; endgame_start = 0.0,only_nonsingular = true)
method = WarmUp(; compile=true, tracker_options=track_options, endgame_options=end_options)
result_2D = get_steady_states(prob, method, varied, fixed; show_progress=false)
plot_phase_diagram(result_2D, class="stable")
...
@btime $get_steady_states(
    $prob, $method, $varied, $fixed; show_progress=false
) #  747.007 ms (4460916 allocations: 438.87 MiB)
...
@btime $get_steady_states(
    $prob, $method, $varied, $fixed; show_progress=false
) # 734.310 ms (4460118 allocations: 438.81 MiB)
