...
``m \\ddot{x} + γ \\dot{x} + ω_0^2 x = F \\cos(ωt)``
to obtain the response as a function of ``ω``
```julia-repl
# having obtained a Problem object, let's find steady states
julia> range = ParameterRange(ω => range(0.8,1.2,100) ) # 100 parameter sets to solve
...
"""
Prepares an input vector to be parsed to the 2D phase diagram with parameters to sweep
and kwargs
"""
function _prepare_input_params(
  ...
  input_array = collect(Iterators.product(param_ranges..., values(fixed_parameters)...))
...
function _get_raw_solution(
  ...
  if show_progress
    bar = Progress(
      length(parameter_values); dt=1, desc="Solving via $method homotopy ...", barlen=50
    )
  end
...
Run a newton solver on `prob::Problem` starting from the solution `soln`
(indexable by `branch` and `index`).
