...
#  Constructors
```julia
# find and store the symbolic Jacobian
Problem(eom::HarmonicEquation; Jacobian=true)
# ignore the Jacobian for now, compute implicitly later
Problem(eom::HarmonicEquation; Jacobian="implicit")
# use J as the Jacobian (a function that takes a Dict)
Problem(eom::HarmonicEquation; Jacobian=J)
# ignore the Jacobian
Problem(eom::HarmonicEquation; Jacobian=false)
```
"""
...
"""
Compile the Jacobian from `prob`, inserting `fixed_parameters`. Returns a function that
takes a dictionary of variables and `swept_parameters` to give the Jacobian.
"""
