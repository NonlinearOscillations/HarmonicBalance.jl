...
"""
Take a pair of harmonic variable u,v and an eigenvalue λ and eigenvector eigvec_2d of the
Jacobian to generate corresponding Lorentzians.
...
function JacobianSpectrum(res::Result; index::Int, branch::Int, force=false)
  ...
  if force
    # if the solution is unstable, return empty spectra
    res.classes["stable"][index][branch] || return all_spectra
  else
...
function _simplify_spectrum(s::JacobianSpectrum)
  ...
  for ω in unique(ω0s)
    ...
    # there are peaks of multiple linewidths at the same frequency => not simplifiable
    length(unique(getfield.(peaks, Symbol("Γ")))) > 1 && return (s)
     # new height is the sum of squares
    total_height = sqrt(sum(getfield.(peaks, Symbol("A")) .^ 2))
