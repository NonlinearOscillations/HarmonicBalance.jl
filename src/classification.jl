...
Creates a solution class in `res` using the inequality `condition`
...
Hopf-unstable solutions are real and have exactly two Jacobian eigenvalues with
positive real parts, which are complex conjugates of each other.
`im_tol` : an absolute threshold to distinguish real/complex numbers.
"""
...
function _is_Hopf_unstable(soln, J)
  ...
  (length(unstable) == 2 && abs(conj(unstable[1]) - unstable[2]) < IM_TOL && return true) ||
    return false
...
"""
$(TYPEDSIGNATURES)
Create binary classification of the solutions, such that each solution points receives
an identifier based on its permutation of stable branches (allows to distinguish between
different phases, which may have the same number of stable solutions).
It works by converting each bistring `[is_stable(solution_1),is_stable(solution_2),...,]`
into unique labels.
"""
...
function filter_result!(res::Result, class::String)
  ...
  # binary_labels stores one Int per parameter set, ignore here
  for c in filter(x -> x != "binary_labels", keys(res.classes))
...
function _classify_default!(result)
  ...
  # shuffle the branches to have relevant ones first
  order_branches!(result, ["physical", "stable"])
  # assign binaries to solutions depending on which branches are stable
  return classify_binaries!(result)
end
