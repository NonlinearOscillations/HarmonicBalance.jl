...
The x,y,z arguments are Strings compatible with Symbolics.jl, e.g.,
`y=2*sqrt(u1^2+v1^2)` plots the amplitude of the first quadratures multiplied by 2.
...
plotted by using the keyword `cut` were it takes a `Pair{Num, Float64}` type entry.
...
function plot1D(
  ...
  if class == "default"
    ...
    if not_class == [] # plot stable full, unstable dashed
      ...
      plot1D(
        res; args..., class="physical", not_class="stable", add=true, style=:dash, kwargs...
      )
      return p
    else
...
function plot2D_cut(
  ...
  if !(cut_par_index ∈ 1:length(res.swept_parameters[cut_par]))
    throw(
      ArgumentError("The value $cut_value is not found in the swept range of $cut_par.")
    )
  end
  ...
  # colouring is matched to branch index - matched across plots
  # skip NaN branches but keep indices
  for k in findall(branch -> !all(isnan.(branch)), branch_data)
...
function plot_spaghetti(
  ...
  if class == "default"
    if not_class == [] # plot stable full, unstable dashed
      ...
      p = plot_spaghetti(
        res; x=x, y=y, z=z, class="physical", not_class=not_class, add=add, kwargs...
      )
      return p
    end
  end
