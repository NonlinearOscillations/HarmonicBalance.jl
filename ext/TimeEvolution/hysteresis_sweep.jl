import HarmonicBalance: plot_1D_solutions_branch, follow_branch
...
function _closest_branch_index(res::Result, state::Vector{Float64}, index::Int64)
    ...
    stable = HarmonicBalance._apply_mask(
        res.solutions, HarmonicBalance._get_mask(res, ["physical", "stable"], []))
...
"""
function follow_branch(starting_branch::Int64, res::Result; y = "u1^2+v1^2",
        sweep = "right", tf = 10000, ϵ = 1e-4)
    ...
    for i in 2:length(Ys)
        ...
        if !isnan(s) # the solution is not unstable or unphysical
            ...
            var_values_noise = real.(getindex.(Ref(sol_dict), var)) .+ 0.0im .+
                               ϵ * rand(length(var))
            for (i, v) in enumerate(var)
                sol_dict[v] = var_values_noise[i]
            end

            problem_t = OrdinaryDiffEq.ODEProblem(
                res.problem.eom, sol_dict, timespan = (0, tf))
...
function plot_1D_solutions_branch(starting_branch::Int64, res::Result;
        x::String, y::String, sweep = "right", tf = 10000, ϵ = 1e-4,
        class = "default", not_class = [], kwargs...)
    p = plot(res; x = x, y = y, class = class, not_class = not_class, kwargs...)

    followed_branch, Ys = follow_branch(
        starting_branch, res, y = y, sweep = sweep, tf = tf, ϵ = ϵ)
    Y_followed = [Ys[param_idx][branch]
                  for (param_idx, branch) in enumerate(followed_branch)]
    X = real.(res.swept_parameters[HarmonicBalance._parse_expression(x)])

    Plots.plot!(p, X, real.(Y_followed); linestyle = :dash, c = :gray,
        label = sweep * " sweep", HarmonicBalance._set_Plots_default..., kwargs...)
    return p
end
