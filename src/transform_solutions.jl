...
Additional substitution rules can be specified in `rules` in the format
`("a" => val)` or `(a => val)`
"""
function transform_solutions(res::Result, func; branches=1:branch_count(res), realify=false)
  ...
  batches = Iterators.partition(
    CartesianIndices(res.solutions), ceil(Int, length(res.solutions) / Threads.nthreads())
  )
...
"""
Parse `expr` into a Symbolics.jl expression, substitute with `rules` and build a function
taking free_symbols
"""
...
Return an array of bools to mark solutions in `res` which fall into `classes` but not
`not_classes`. Only `branches` are considered.
"""
...
Transform a solution into the lab frame (i.e., invert the harmonic ansatz) for the natural
variable `x` for `times`. You can also compute the velocity by passing `d(x,t)` for the
natural variable `x`.
Either extract the solution from `res::Result` by `index` and `branch` or input
`soln::OrderedDict` explicitly.
"""
