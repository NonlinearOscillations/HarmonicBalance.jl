...
Saves `object` into `.jld2` file `filename`(the suffix is added automatically
if not entered). The resulting file contains a dictionary with a single entry.
...
the symbolic variables are reinstated in the `HarmonicBalance` namespace.
