...
function HarmonicBalance.steady_state_sweep(
  ...
  foreach(pairs(sweep_range)) do (i, value)
    ...
    if !isapprox(zeros, 0; atol=1e-5) || any(λ -> λ > 0, real.(eigval))
      sol_ss = solve(remake(prob_ss; p=parameters, u0=u0), alg_ss; abstol=1e-5, reltol=1e-5)
      result[i] = sol_ss.u
    else
      result[i] = sol_nn.u
    end
...
function get_new_parameters(prob, varied_idx, value)
  ...
  if hasfield(typeof(prob.p), :tunable)
    ...
    parameters = eltype(prob.p)[i == varied_idx ? value : x for (i, x) in enumerate(prob.p)]
  end
  return parameters
end
